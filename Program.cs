...
namespace Lab6
{
    class Program
    {
        ...
        {
            using (var ctx = new ITCompanyContext())
               
            {
                ...
                switch (action)
                {
                    case 1:
                        if(table == 1)
                        {
                            ctx.Projects.Add(new Project() { ProjectName = "Trynow", CustomerName = "John", StartDate = DateTime.Now });
                     
                        } else
                        {
                            ctx.People.Add(new Person() { FirstName = "Olena", LastName = "Hul", Role = "Python developer" });
                        }
                        ctx.SaveChanges();
